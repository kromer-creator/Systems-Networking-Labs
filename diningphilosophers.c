...
/*
 * Here is the dining philosopher's problem. It is described in great
...
int pickup_forks(int philosopher_number, time_t *last_meal)
{
    ...
    //Odd-numbered philosophers grab right fork first
    if (forks[(philosopher_number)%2]) {
        if (forks[(philosopher_number + 1) % 5] < 1) {
            //Lock critical portion
            pthread_mutex_lock(&fork_lock[(philosopher_number + 1 % 5)]);
    ...
    //Even-numbered philosophers grab left fork first
    else {
        ...
        if (forks[philosopher_number] < 1) {
            //Lock critical portion
            pthread_mutex_lock(&fork_lock[(philosopher_number)]);
...
void return_forks(int philosopher_number)
{
    ...
    //Unlock locks after forks are returned
    pthread_mutex_unlock(&fork_lock[(philosopher_number+1%5)]);
    pthread_mutex_unlock(&fork_lock[(philosopher_number)]);
    forks[(philosopher_number + 1)%5]++;
    forks[philosopher_number]++;
}
